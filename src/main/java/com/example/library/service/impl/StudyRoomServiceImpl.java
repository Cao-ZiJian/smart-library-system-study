...
import static com.example.library.config.RedisConfig.CACHE_STUDY_ROOM_ALL;
...
public class StudyRoomServiceImpl extends ServiceImpl<StudyRoomMapper, StudyRoom> implements StudyRoomService {
    ...
    @Caching(evict = {
            ...
            @CacheEvict(cacheNames = CACHE_STUDY_ROOM_ALL, allEntries = true)
    })
    ...
    @Override
    @Cacheable(cacheNames = CACHE_STUDY_ROOM_ALL, key = "'all'")
    ...
    @Override
    @Cacheable(cacheNames = CACHE_STUDY_ROOM_ENABLED, key = "'enabled'")
    ...
    @Override
    @Cacheable(cacheNames = CACHE_STUDY_ROOM_ENABLED, key = "'detail:' + #id")
