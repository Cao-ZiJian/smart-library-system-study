...
public class AuthController {
     ...
     * 用户注册
     */
    @Operation(summary = "注册普通用户：密码使用 BCrypt 加密保存")
     ...
     */
    @Operation(summary = "退出登录：删除 Redis Session 并将当前 jti 加入黑名单")
    @PostMapping("/logout")
     ...
     * 获取当前登录用户信息
     */
    @Operation(summary = "获取当前登录用户：基于认证拦截器写入的 UserContext")
    @GetMapping("/me")
     ...
     */
    @Operation(summary = "维护当前用户头像地址：头像图片可先通过基础上传接口获取 URL")
    @PutMapping("/avatar")
