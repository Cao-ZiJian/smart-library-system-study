...
public class BorrowManageController {
    ...
    @ApiOperation("审核借阅申请：APPLYING 流转为 APPROVED 或 REJECTED")
    @PostMapping("/approve")
    ...
    @ApiOperation("确认出借：状态流转为 LENT，并条件扣减库存")
    @PostMapping("/lend/{id}")
    ...
    @ApiOperation("确认归还：LENT 或 OVERDUE 流转为 RETURNED，并恢复库存")
    @PostMapping("/return")
    ...
    @ApiOperation("借阅流程记录分页：用于馆员处理申请、出借和归还")
    @GetMapping("/page")
