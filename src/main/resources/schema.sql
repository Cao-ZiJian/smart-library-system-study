...
CREATE TABLE `user` (
    ...
    `token_version` INT         NOT NULL DEFAULT 0 COMMENT '会话版本号，递增后旧登录态失效',
    `avatar_url`   VARCHAR(1024)         DEFAULT NULL COMMENT '头像图片 URL',
...
CREATE TABLE `book` (
    ...
    `cover_url`      VARCHAR(1024)         DEFAULT NULL COMMENT '封面图片 URL',
...
CREATE TABLE `study_room` (
    ...
    `image_url`    VARCHAR(1024)         DEFAULT NULL COMMENT '自习室展示图 URL',
    `status`       TINYINT      NOT NULL DEFAULT 1 COMMENT '状态：1启用 0禁用',
    `create_time`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`)
...
CREATE TABLE `reservation` (
    ...
    KEY `idx_reservation_seat_status_time` (`seat_id`, `status`, `start_time`, `end_time`)
